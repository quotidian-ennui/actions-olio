...
|  body_path   | string |  false   |                                                                       |                     A file location that contains the body to be used as the comment                     |
|  edit_mode   | string |  false   |                              `"replace"`                              |     The edit mode to use when updating an existing comment. Valid values are 'replace' and 'append'.     |
|              |        |          |                                                                       |                                  If not set, the default is 'replace'.                                   |
| issue_number | string |  false   |                                                                       |              The issue number to use, if not set will try and use value in github context.               |
|  repository  | string |  false   |                     `"${{ github.repository }}"`                      |          Override the repository (your token needs to allow it) default is 'github.repository'           |
| search_term  | string |  false   | `"pr-comment: workflow=${{ github.workflow }} job=${{ github.job }}"` | The search term that allows us to update an existing comment; if not found then a new comment is created |
|              |        |          |                                                                       |                                                                                                          |
|    token     | string |  false   |                        `"${{ github.token }}"`                        |                             The GITHUB token to use default is github.token                              |
...
|------------|--------|----------------------------------------------------|
| comment_id | string | The ID of the comment that was created or updated. |
